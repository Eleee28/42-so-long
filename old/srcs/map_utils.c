...
/*   Updated: 2024/07/02 09:56:43 by ejuarros         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fill_with_ones(char **map, int row, int col)
{
	...
	ret = 0;
	if (row < 0 || (size_t)row >= ft_len_matrix(map) || col < 0
		|| (size_t)col >= ft_strlen(map[0]) || map[row][col] == '1')
		return (0);
	if (map[row][col] == 'E')
		ret = 1;
	map[row][col] = '1';
	ret = fill_with_ones(map, row - 1, col) || ret;
	ret = fill_with_ones(map, row + 1, col) || ret;
	ret = fill_with_ones(map, row, col - 1) || ret;
	ret = fill_with_ones(map, row, col + 1) || ret;
	return (ret);
}
