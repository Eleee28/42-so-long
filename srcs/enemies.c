...
void print_enemies(t_game *game, t_pos pos)
{
	...
	while (i < game->enemies.n_enemies)
	{
		if (equal_pos(pos, game->enemies.enemies[i]))
			mlx_put_image_to_window(game->mlx.mlx_data.mlx_ptr, game->mlx.mlx_data.mlx_win, game->mlx.sprites.enemy.down, pos.y * IMG_W, (pos.x + 1) * IMG_H);
		i++;
	}
}

// https://old.chuidiang.org/clinux/funciones/rand.php -> info on srand() and rand()
void    move_enemies(t_game *game) {
    int m;
    int i;
    t_pos aux;

    m = rand() % 5;     // generates a random number between 0 and 4
